eval(`RL=readline,P=s=>s.split(" "),[W,H]=P(RL()).map(Number),G=[]
for(i=0;i<H;i++)G.push(P(RL()).map(Number))
E=+RL(),TB="010123010123000130013011000101201021100010",gp=t=>t<2?[t]:t<4?[2,3]:t<6?[4,5]:t<10?[6,7,8,9]:[10,11,12,13],PL=[],F=(x,y,e,u,p)=>{
if(y>=H)return x==E
...
A="WAIT"
free=S.every((s,j)=>s[3]>=j+2),done=0
if(S.length&&!free){D();done=+(A!="WAIT")}
if(!done&&DF){
if(O[DF[1]][DF[0]]==DF[2])DF=null
else if(!K(DF[0],DF[1])&&!(DF[0]==X&&DF[1]==J)){z=Y.find(c=>c[0]==DF[0]&&c[1]==DF[1]);if(!z||+TB[z[2]*14+DF[2]]){A=U(DF[0],DF[1],DF[2]);done=1}else DF=null}}
if(!done&&!DF){
bk=999,bc=null
for(r of RK){
tr=TJ(r[0],r[1],r[2])
kc=0
for(k=1;k<tr.length;k++){ip=Y[k];if(!ip)break
if(tr[k][0]==ip[0]&&tr[k][1]==ip[1]){kc=k;break}
pp=Y[k-1];if(pp&&tr[k][0]==pp[0]&&tr[k][1]==pp[1]&&tr[k-1][0]==ip[0]&&tr[k-1][1]==ip[1]){kc=k;break}}
if(!kc||kc>=bk)continue
for(m=1;m<kc;m++){cx=tr[m][0],cy=tr[m][1],ce=tr[m][2]
if(G[cy][cx]<0||cx==X&&cy==J||cx==E&&cy==H-1||K(cx,cy))continue
g=gp(O[cy][cx]),L=g.length;if(L<2)continue
if(Y.some(c=>c[0]==cx&&c[1]==cy))continue
bd=9,bt=-1
for(tt of g)if(+TB[ce*14+tt]==0){qq=(tt-O[cy][cx]+L)%L,st=qq<L-qq?qq:L-qq;if(st<bd){bd=st;bt=tt}}
...
cx=tr[k][0],cy=tr[k][1],ce=tr[k][2],g=gp(O[cy][cx]),L=g.length,np=Y[k+1],bd=9,bt=-1
if(G[cy][cx]<0||cx==X&&cy==J||cx==E&&cy==H-1||K(cx,cy))break
for(tt of g){id=+TB[Y[k][2]*14+tt];if(+TB[ce*14+tt]==0&&id&&(!np||id==1&&np[0]==cx&&np[1]==cy+1||id==2&&np[0]==cx-1&&np[1]==cy||id==3&&np[0]==cx+1&&np[1]==cy)){qq=(tt-O[cy][cx]+L)%L,st=qq<L-qq?qq:L-qq;if(st<bd){bd=st;bt=tt}}}
if(bt>=0){DF=[cx,cy,bt];A=U(cx,cy,bt);done=1}break}}}}
if(!done)D()
a=P(A),nn=Y[1];if(a[2]&&nn&&+a[0]==nn[0]&&+a[1]==nn[1]&&!+TB[nn[2]*14+O[+a[1]][+a[0]]]){Z(+a[0],+a[1],a[2]=="LEFT");A="WAIT"}
print(A)}`)