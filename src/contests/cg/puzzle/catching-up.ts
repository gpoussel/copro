...
for (let i = 0; i < 10; i++) {
    ...
    for (let j = 0; j < 10; j++) {
        const c = row[j];
        if (c === 'L' || c === 'P') { playerY = i; playerX = j; grid[i][j] = '-'; }
        if (c === 'l' || c === 'E' || c === 'd') { grid[i][j] = '-'; }
    }
}
...
function enemyBestMove(ey: number, ex: number, py: number, px: number): [number, number] {
    let bestDist = -1, bestY = ey, bestX = ex;
    // Try stay + 4 dirs
